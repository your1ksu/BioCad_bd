...
# Bootstrap and run multiple_alignment.py in a conda environment with MAFFT.
#
# Usage:
...
MSA_PY="${MSA_PY:-${SCRIPT_DIR}/multiple_alignment.py}"
MINIFORGE_DIR="${MSA_MINIFORGE_DIR:-${SCRIPT_DIR}/.miniforge}"

if [[ ! -f "${MSA_PY}" ]]; then
    echo "ERROR: multiple_alignment.py not found: ${MSA_PY}" >&2
    echo "Set MSA_PY=/path/to/multiple_alignment.py or place this script next to multiple_alignment.py." >&2
    exit 1
fi
...
main() {
    ...
    echo "Running multiple_alignment.py in environment '${ENV_NAME}'..."
