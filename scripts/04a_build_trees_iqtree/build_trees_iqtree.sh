...
for fasta in "${fasta_files[@]}"; do
    ...
    # Check number of sequences in FASTA
    nseqs=$(grep -c "^>" "$fasta" || echo 0)
    if [ "$nseqs" -lt 4 ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Skipping $basename: only $nseqs sequences (need ≥4 for bootstrap)"
        continue
    fi

    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Building tree for $basename ($nseqs seqs) ..."
    iqtree \
        -s "$fasta" \
        -m MFP \
        -B 1000 \
