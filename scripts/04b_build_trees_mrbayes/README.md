...
переименовываются в `T0001…`. Этот файл нужен `../05_clade_search/clade_search.py`,
чтобы вернуть исходные id в отчёте.

## Запуск

```bash
python build_trees_mrbayes.py                       # aligned_sequences/ → mrbayes/
python build_trees_mrbayes.py путь/до/fasta --out mrbayes
python build_trees_mrbayes.py --nexus-only           # только сгенерировать .nex, не запускать mb
python build_trees_mrbayes.py --outgroup <seq_id>    # если в fasta есть germline-предок
```
...
`mrbayes/<группа>.nex.con.tre` — вход для [../05_clade_search/clade_search.py](../05_clade_search/clade_search.py)
