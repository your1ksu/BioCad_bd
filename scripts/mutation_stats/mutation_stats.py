#!/usr/bin/env python3
"""Локализация мутаций по областям FR/CDR и оценка отношения dN/dS.

Вход:  mutations.tsv (выход scripts/analyze_mutations/analyze_mutations.py)
       либо каталог mutation_tables/ — тогда все mutations.tsv обрабатываются
       рекурсивно с агрегацией результатов.
Выход: таблица в stdout; при --tsv <path> — файл TSV.

Вычисляются две характеристики:

1. Распределение мутаций по областям FR1-FR4 и CDR1-CDR3.

2. Отношение dN/dS методом Ней — Годзёбори с поправкой Джукса — Кантора.
   Значение > 1 соответствует положительному отбору (ожидается в CDR,
   определяющих связывание антигена), < 1 — очищающему отбору (ожидается в
   каркасных областях FR).

Расчёт dN/dS требует наличия синонимных замен во входной таблице как
знаменателя dS. В версиях analyze_mutations.py, где отбор мутаций выполняется
по условию `q_aa != s_aa`, синонимные замены не регистрируются и dN/dS не
определён; скрипт диагностирует этот случай явно. Соответствующее изменение
условия отбора описано в readme.md.

Ограничение: N_sites и S_sites вычисляются по кодонам ref_codon наблюдаемых
мутаций, поскольку полная последовательность зародышевой линии в mutations.tsv
отсутствует. См. раздел «Ограничения» в readme.md.
"""
...
def format_report(stats: dict[str, RegionStats], n_seqs: int, title: str) -> list[str]:
    ...
    if total_s == 0 and total_muts > 0:
        lines.append("dN/dS не определён: во входной таблице отсутствуют синонимные замены")
        lines.append("(is_silent='no' во всех строках), знаменатель dS пуст. Причина —")
        lines.append("условие отбора мутаций `q_aa != s_aa` в analyze_mutations.py, при")
        lines.append("котором регистрируются только замены, изменяющие аминокислоту.")
        lines.append("См. раздел «Требование к входным данным» в readme.md.")
    else:
        lines.append("dN/dS > 1 — положительный отбор (ожидается в CDR);")
        lines.append("dN/dS < 1 — очищающий отбор (ожидается в FR).")
        lines.append("N_sites и S_sites оценены по ref_codon наблюдаемых мутаций;")
        lines.append("см. раздел «Ограничения» в readme.md.")
    return lines
