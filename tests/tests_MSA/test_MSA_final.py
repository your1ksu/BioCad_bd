...
MSA_DIR = Path(__file__).resolve().parents[1]
...
class TestMsaFinal(unittest.TestCase):
    def test_discover_fasta_files_recursively(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            ...
            result = MSA_final.discover_fasta_files(input_dir)
    ...
    def test_find_mafft_uses_explicit_path(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(MSA_final.find_mafft(mafft_bin), mafft_bin)
    ...
    def test_run_mafft_writes_alignment(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            ...
            with patch("MSA_final.subprocess.run", side_effect=fake_mafft):
                MSA_final.run_mafft(Path("mafft"), input_fasta, output_fasta)
    ...
    def test_main_aligns_files_and_writes_manifest(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            ...
            with patch.object(sys, "argv", test_argv), patch(
                "MSA_final.subprocess.run", side_effect=fake_mafft
            ):
                MSA_final.main()
